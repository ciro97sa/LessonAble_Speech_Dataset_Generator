...
def get_audiosentence_objects(author, language, filename, starting_count):
    ...
    for (index,line) in enumerate(non_empty_lines):
      ...
      if (line.strip().isdecimal() and index != len(non_empty_lines)-1):
        ...
        audio_title = filename + str(index)
        ...
        if ((secondSentence.rstrip().endswith('.') or secondSentence.rstrip().endswith('?')) or (secondSentence == '' and (firstSentence.rstrip().endswith('.') or firstSentence.rstrip().endswith('?')))):
          ...
          final_sentence = text_num_2_str(final_sentence, language)
          audio_title = filename + str(count)

          # set
          final_obj = Audio_Sentence(author, final_sentence, audiosentence_tmp_array[0].startTime, audiosentence_tmp_array[len(audiosentence_tmp_array) - 1].endTime)
...
def produceSpeechDataset(author, language, source_audio_path ,raw_dataset_path, output_path):
    ...
    for subdirectory in subdirectories:
      ...
      for filename in os.listdir(raw_dataset_path + subdirectory):
        ...
        if filename.lower().endswith('.mp4'):
          extract_audio(root+subdirectory+ '/'+filename)
        # get the .srt file and getting the SentenceObjects.
        if filename.lower().endswith('.srt'):
          subtitle_objects = get_audiosentence_objects(author, language, root+subdirectory+'/'+filename, wavs_count)
      ...
      for obj in subtitle_objects:
        ...
        obj.write_audiosentence(source_audio_path, wavs_path)
